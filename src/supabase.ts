...
/**
 * Canonical customer service adapter.
 * Manchant is the source of truth: businessTemplate.offerings is the public
 * service contract. No business-type or service-name allow-list lives here.
 * Legacy games/services are only compatibility inputs until Manchant publishes
 * them as offerings. The normalized offerings are also exposed as JSONB
 * products so the existing customer catalog and realtime renderer consume the
 * exact same contract without a second service UI implementation.
 */
function normalizeStoreServices(store: any): any {
  ...
  const serviceProducts = canonical
    ...
    .map((o: any, index: number) => ({
      ...
      name: String(o.name || 'Service'),
      description: o.description || '',
      sellingPrice: Number(o.price ?? o.sellingPrice ?? o.selling_price ?? 0),
      selling_price: Number(o.price ?? o.sellingPrice ?? o.selling_price ?? 0),
      quantity: 999999,
      unit: o.unit || (o.pricing === 'time' ? 'session' : 'service'),
      isService: true,
      turnaround: o.turnaround || '',
      category: o.category || 'Services',
      image: o.image || '',
      discontinued: false,
      status: 'active',
      servicePricing: o.pricing || 'fixed',
      icon: o.icon || '',
    }))
...
export const supabase = new Proxy(baseSupabase, { get(target, property, receiver) {
  ...
  return ((...args: RpcArgs) => {
    ...
    if (fn === 'place_order_atomic' && args[1] && typeof args[1] === 'object' && !Array.isArray(args[1])) {
      ...
      rpcArgs = [fn, { ...params, p_customer_uuid: params.p_customer_uuid ?? null, p_is_guest: params.p_is_guest ?? true }] as unknown as RpcArgs;
    }
    const request = originalRpc(...rpcArgs);
    return request.then(async (result) => {
      if (fn !== 'place_order_atomic') return result;
      if (!result.error && result.data) { if (typeof result.data === 'string' && !result.data.startsWith('offline-')) void notifyMerchantOfNewOrder(target, String(result.data)); }
      else if (looksLikeNetworkFailure(result.error)) return queueOfflineOrder(rpcArgs);
      return result;
    }, async (error: unknown) => { if (fn !== 'place_order_atomic' || !looksLikeNetworkFailure(error)) throw error; return queueOfflineOrder(rpcArgs); });
...
if (typeof window !== 'undefined') {
  window.addEventListener('online', async () => {
    ...
    for (const item of queue) { try { const result = await originalRpc(...item.args); if (result.error) remaining.push(item); else if (result.data) void notifyMerchantOfNewOrder(baseSupabase, String(result.data)); } catch { remaining.push(item); } }
