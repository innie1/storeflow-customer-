-- Additive rollout stage. Install these token-returning endpoints before the
-- customer bundle switches to them. Legacy endpoints remain available until
-- 20260912111500_secure_token_checkout.sql is applied after deployment.
...
begin
  ...
  select coalesce(data, '{}'::jsonb) into v_store_data
  from public.stores
  where id = v_store_id and coalesce(subscription_status, 'active') = 'active';
  ...
  if v_delivery_type = 'delivery' then
    ...
    if v_free_delivery_threshold > 0 and v_subtotal >= v_free_delivery_threshold then v_delivery_fee := 0; end if;
  end if;
  ...
  if v_minimum_order > 0 and v_subtotal < v_minimum_order then raise exception 'Minimum order is %', v_minimum_order; end if;
  ...
  if v_requested_loyalty then
    ...
    if coalesce((v_loyalty->>'enabled')::boolean, false) then
      if coalesce(v_loyalty->>'earnPerHundred', '') ~ '^[-+]?[0-9]+([.][0-9]+)?$' then v_earn_rate := greatest(0, (v_loyalty->>'earnPerHundred')::numeric); end if;
      if coalesce(v_loyalty->>'redeemThreshold', '') ~ '^[0-9]+$' then v_redeem_threshold := greatest(1, (v_loyalty->>'redeemThreshold')::integer); end if;
      if coalesce(v_loyalty->>'redeemValueNaira', '') ~ '^[-+]?[0-9]+([.][0-9]+)?$' then v_redeem_value := greatest(0, (v_loyalty->>'redeemValueNaira')::numeric); end if;
      select coalesce(sum(total), 0) into v_completed_total from public.orders where store_id = v_store_id and customer_phone = p_customer_phone and status = 'Completed';
      v_earned := floor((v_completed_total / 100) * v_earn_rate);
      select coalesce(sum(points_redeemed), 0) into v_redeemed from public.loyalty_redemptions where store_id = v_store_id and customer_phone = p_customer_phone;
  ...
  v_notes := v_notes || jsonb_build_object('online_discount', v_online_discount, 'delivery_fee', v_delivery_fee, 'loyalty_discount', v_loyalty_discount, 'server_pricing', true);

  v_order_id := public.place_order_atomic(p_store_id, p_customer_name, p_customer_phone, p_order_number, 'Pending', v_subtotal, v_total, v_notes::text, p_items, p_customer_uuid, p_is_guest);
  update public.orders set discount = round(v_online_discount + v_loyalty_discount, 2) where id = v_order_id;
  ...
  if v_access_token is null then raise exception 'Secure checkout could not create an order token'; end if;
  return jsonb_build_object('order_id', v_order_id, 'access_token', v_access_token, 'status', 'Pending', 'subtotal', v_subtotal, 'discount', round(v_online_discount + v_loyalty_discount, 2), 'delivery_fee', v_delivery_fee, 'total', v_total);
end;
$$;
...
declare
  ...
  v_balance jsonb;
begin
  select id, store_id, customer_phone, access_token into v_order
  from public.orders where id = p_order_id and access_token = p_access_token;
  if not found then raise exception 'Not authorized to view this order'; end if;
  select * into v_redemption from public.loyalty_redemptions where order_id = p_order_id order by created_at desc limit 1;
  if not found then return jsonb_build_object('success', false, 'remainingPoints', null); end if;
  v_balance := public.get_customer_loyalty_balance(v_order.store_id, v_order.customer_phone);
  return jsonb_build_object('success', true, 'valueNaira', v_redemption.value_naira, 'pointsRedeemed', v_redemption.points_redeemed, 'remainingPoints', (v_balance->>'points')::integer);
end;
$$;
